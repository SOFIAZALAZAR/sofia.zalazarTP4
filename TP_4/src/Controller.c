 ...
 * \param pArrayListEmployee LinkedList* puntero a la lista de empleados
 * \param pId int* puntero al id
 * \param pArrayBaja LinkedList* puntero a la lista de baja
 * \return int retorna 0 si algun puntero es nulo o si el usuario no confirma la baja,
 *                     1 si la baja esta ok
 *
 */
int controller_removeEmployee(LinkedList* pArrayListEmployee,int* pId,LinkedList* pArrayBaja)
{
    int todoOk = 0;
    int id;
    char confirma;
    ...
    if(pArrayListEmployee!=NULL && pId!=NULL)
    {
        ...
        if(id>0 && id<*pId)
        {
            ...
            if(confirma=='s')
            {
                ...
                if(aux!=NULL && !ll_add(pArrayBaja,aux))
                {
                    aux=NULL;
                    todoOk = 1;
                }
...
/** \brief vuelve a dar de alta un empleado dado de baja
 *
 * \param pArrayListEmployee LinkedList* puntero a la lista
 * \param pArrayBaja LinkedList* puntero a la lista de baja
 * \return int retorna 0 si alguno de los punteros es nulo o el id no es valido,
 *                     1 si el alta es extisa
 *
 */
int controller_addEmployeeRemove(LinkedList* pArrayListEmployee,LinkedList* pArrayBaja)
{
    int todoOk = 0;
    int id;
    char confirma;
    ...
    Employee* aux = NULL;
    Employee* emp = NULL;
    if(pArrayListEmployee!=NULL && pArrayBaja!=NULL)
    {
        controller_ListEmployee(pArrayBaja);
        printf("Ingrese el Id del empleado que desea dar de alta: ");
        fflush(stdin);
        scanf("%d",&id);
        if(id>0)
        {
            for(int i=0; i<ll_len(pArrayBaja); i++)
            {
                aux = (Employee*)ll_get(pArrayBaja,i);
                if(aux->id == id)
                {
                    break;
                }
            ...
            if(confirma=='s')
            {
                indice = ll_indexOf(pArrayBaja,aux);
                aux =(Employee*) ll_pop(pArrayBaja,indice);
                if(aux!=NULL)
                {
                    for(int i=0; i<ll_len(pArrayListEmployee); i++)
                    {
                    ...
                    if(indice>-1 && !ll_push(pArrayListEmployee,indice,aux))
                    {
                        aux=NULL;
                        emp=NULL;
                        todoOk = 1;
...
/** \brief Hace una copia de la lista y la guarda en un archivo
 *
 * \param pArrayListEmployee LinkedList* puntero a la lista
 * \param pArrayClone LinkedList* puntero a la lista duplicada
 * \param path char* nombre del archivo
 * \return int retorna 0 si alguno de los punteros es nulo o falla la carga del archivo
 *                      1 si se guarda bien;
 */
int controller_cloneList(LinkedList* pArrayListEmployee,LinkedList* pArrayClone,char* path)
{
    int todoOk=0;
    if(pArrayListEmployee!=NULL)
    {
        pArrayClone = ll_clone(pArrayListEmployee);
        if(pArrayClone!=NULL && ll_containsAll(pArrayClone,pArrayListEmployee)==1)
        {
            todoOk = controller_saveAsText(path,pArrayClone);
        }
